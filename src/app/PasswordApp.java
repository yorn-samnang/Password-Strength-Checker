...
//
//        // Create storage with user-provided master password
//        PasswordStorage storage = new PasswordStorage(
//                "src/resources/passwords.txt",
//                masterPassword
//        );
//
...
public class PasswordApp {
    ...
    public static void main(String[] args) {
        ...
        // 🔐 ASK USER FOR MASTER PASSWORD AT START
        System.out.print("Enter master password: ");
        String masterPassword = sc.nextLine().trim();
        ...
        // Optional validation
        if (!storage.validateMasterPassword()) {
            System.out.println("⚠️ Master password may be wrong or storage is empty.");
        }
        ...
        while (true) {
            ...
            switch (choice) {
                ...
                case 2 -> handlePasswordView(storage);
                case 3 -> handlePasswordExport(sc, storage);
                case 4 -> handlePasswordGeneration(sc, masterPassword);
                case 5 -> handlePasswordGuidelines(guidelines);
                case 6 -> {
                    System.out.println("Goodbye!");
                    return;
    ...
    private static void handlePasswordCheck(Scanner sc, PasswordChecker checker, PasswordStorage storage) {
        ...
        if (strength.equalsIgnoreCase("Strong")) {
            storage.savePassword(username, passwordInput);
        } else {
            System.out.println("⚠️ Try to create a stronger password.");
        }
    }
    ...
    private static void handlePasswordView(PasswordStorage storage) {
        try {
            ...
            for (Password p : list) {
                ...
                System.out.println("ID        : " + p.getId());
                System.out.println("Username  : " + p.getUsername());
                System.out.println("Password  : " + p.getDecryptedPassword());
                System.out.println("Created At: " + p.getCreatedAt());
            }
    ...
    private static void handlePasswordGeneration(Scanner sc, String masterPassword) {
        System.out.println("Generating a strong password...");
        String generated = PasswordGenerator.generateStrongPassword();
